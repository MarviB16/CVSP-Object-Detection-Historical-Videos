...
def evaluate(
    ...
    for label in range(generator.num_classes()):
        ...
        for i in range(m):
            for j in range(n):
                if n*i+j+1 > 2: break
                cells[i, j].plot(recall, precision, color='blue', linewidth=1.0)
                cells[i, j].set_xlabel('recall', fontsize=14)
                cells[i, j].set_ylabel('precision', fontsize=14)
                cells[i, j].grid(True)
                cells[i, j].set_xticks(np.linspace(0,1,11))
                cells[i, j].set_yticks(np.linspace(0,1,11))
                cells[i, j].set_title(label_text+", AP: {:.3f}".format(average_precision), fontsize=16)
        plt.savefig(label_text+'_prc_retinanet.png', bbox_inches='tight')
